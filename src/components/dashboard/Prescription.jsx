      ...
      <select onChange={(e) => setSelectedPatientId(e.target.value)} value={selectedPatientId}>
        ...
        {confirmedAppointments.map((appt) => (
          <option key={appt.id} value={appt.patientId}>
            {appt.patientName} - {new Date(appt.doctorResponse.dateTime).toLocaleString()}
          </option>
        ))}
      </select>

      <h3>Enter Prescription Details:</h3>
      <input type="text" name="medicine" value={prescriptionData.medicine} onChange={handlePrescriptionChange} placeholder="Medicine Name" />
      <input type="text" name="dosage" value={prescriptionData.dosage} onChange={handlePrescriptionChange} placeholder="Dosage" />
      <textarea name="instructions" value={prescriptionData.instructions} onChange={handlePrescriptionChange} placeholder="Instructions"></textarea>
